...
motion:
  duration-micro: 150ms # press tombol, ikon tab
  duration-base: 250ms  # indicator dock, back button
  duration-page: 300ms  # transisi halaman/route
  duration-sweep: 700ms # donut chart, onboarding step
  curve-standard: "cubic-bezier(0.65, 0, 0.35, 1)" # easeInOutCubic
  curve-emphasis: "cubic-bezier(0.05, 0.7, 0.1, 1.0)" # decelerate halus tanpa bounce
...
components:
  ...
  back-button:
    size: 44px
    textColor: "{colors.primary}"
    textColorOnDark: "{colors.primary-on-dark}"
    typography: "{typography.caption-strong}"
...
### Principles
- **Pendek & Konsisten**: Durasi animasi berkisar antara 150ms hingga 300ms (kecuali sweep visualisasi grafik gizi 700ms). Animasi tidak boleh memperlambat tugas pengguna.
- **Arah Kontinu (Spatial Continuity)**: Pergerakan elemen mengikuti arah fisik yang logis (maju ke kanan/bawah saat membuka detail, mundur ke kiri/atas saat kembali).
- **GPU-Only Performance**: Animasi dibatasi secara ketat pada properti transform (`translate`, `scale`) dan `opacity`. Jangan menganimasikan properti layout (`width`, `height`, `margin`, `padding`) yang memicu reflow.
- **Hormati Preferensi Pengguna (Reduce-Motion)**: Bila pengguna mengaktifkan pengaturan *reduced motion* pada perangkat/browser, seluruh animasi dinonaktifkan secara instan (durasi 0ms atau instant crossfade).

### Hierarchy & Tokens

| Token | Nilai | Pemakaian |
|---|---|---|
| `{motion.duration-micro}` | 150ms | Feedback sentuhan tombol (`scale(0.95)`), transisi ikon tab |
| `{motion.duration-base}` | 250ms | Pergeseran indikator kapsul dock, transisi back button |
| `{motion.duration-page}` | 300ms | Transisi pergantian layar/route, slide step onboarding |
| `{motion.duration-sweep}` | 700ms | Sapuan cincin grafik nutrisi (donut chart), evaluasi meter BMI |
| `{motion.curve-standard}` | `cubic-bezier(0.65, 0, 0.35, 1)` | Kurva standar (easeInOutCubic) untuk transisi elemen antarmuka |
| `{motion.curve-emphasis}` | `cubic-bezier(0.05, 0.7, 0.1, 1.0)` | Kurva deselerasi halus tanpa bounce untuk visualisasi data |

### Surface Specifications
- **Indicator Dock**: Kapsul aktif bergeser mulus secara horizontal selama 250ms (`{motion.duration-base}`, `{motion.curve-standard}`) saat berpindah tab.
- **Transisi Halaman / Routing**: Perpindahan layar detail menggunakan kombinasi slide + fade selama 300ms (`{motion.duration-page}`, `{motion.curve-standard}`). Arah maju saat push, dan berbalik saat pop.
- **Press State Tombol**: Efek micro-interaction `transform: scale(0.95)` dengan durasi 150ms (`{motion.duration-micro}`).
- **Back Button**: Muncul dan menghilang dengan transisi fade + subtle slide saat stack navigasi berubah.
- **Donut Chart Progress Sweep**: Dua cincin nutrisi (kalori & hidrasi) menyapu dari 0° ke nilai target selama 700ms (`{motion.duration-sweep}`, `{motion.curve-emphasis}`) saat layar pertama kali dibuka atau diperbarui.
- **Onboarding Multi-Step**: Pergantian langkah formulir profil gizi menggunakan transisi crossfade + horizontal slide 300ms (`{motion.duration-page}`).
...
**`dock`** — Floating curved dome bottom navigation dock melayang di atas konten pada area utama aplikasi. Tinggi 64px, background `{colors.canvas-parchment}` pada 80% opacity dengan `backdrop-filter: blur` (efek frosted-glass), margin 16px dari tepi dan bawah layar, border 1px `{colors.divider-soft}`, dengan kubah lengkung dinamis (*smooth curved dome* setinggi 16px) yang mengikuti posisi tab aktif. Tanpa bayangan/glow.

**`dock-item` & `dock-item-selected`** — Item tab di dalam dock dengan ukuran ikon 22–24px.
- Item aktif (`dock-item-selected`): berupa *floating circular badge* sirkular 48px terisi `{colors.primary}` solid (tanpa gradient, tanpa glow/shadow), terangkat di pusat kubah lengkung, dengan label teks `{colors.primary}` di bawahnya.
- Item non-aktif (`dock-item`): warna teks/ikon `{colors.ink-muted-48}`. Indikator kubah dan bubble aktif bergeser secara mulus (*smooth horizontal glide*) saat pengguna berpindah tab.

**`back-button`** — Tombol kembali di pojok kiri atas aplikasi. Hit-area 44 × 44px, chevron `{colors.primary}` (pada surface gelap beralih ke `{colors.primary-on-dark}`), label opsional `{typography.caption-strong}`.

> **Naratif Navigasi Aplikasi:**
> - Dock floating frosted 4 tab **hanya tampil di area utama pasca-onboarding** (4 item: **Beranda**, **Nutri Mate**, **Nutri Meal**, **Nutri Doc**). Fitur Nutri Calculator, Nutri Education, dan Profil Pengguna diakses langsung dari dalam Beranda/dashboard tanpa tab terpisah di dock.
> - Dock **tidak tampil** pada alur Onboarding/Auth maupun pada halaman detail (sub-halaman). Halaman detail ditampilkan secara full-bleed menutupi dock.
> - Tombol **Back** (`back-button`) **hanya muncul di kiri atas halaman non-root** (halaman detail/sub-halaman saat stack dapat di-pop). Keempat tab akar utama tidak menampilkan tombol back.
